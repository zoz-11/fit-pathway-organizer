  ...
  try {
    ...
    if (checkRateLimit(user.id, 20)) {
      await logSecurityEvent(
        supabaseClient,
        user.id,
        "goal_management_rate_limit_exceeded",
        { ipAddress: req.headers.get("x-forwarded-for") },
        "medium",
      );
      return new Response(
    ...
    let { action, goal, goalId } = parsedRequest.data;

    // Sanitize goal description if present
    ...
    switch (action) {
      case "create": {
        if (!goal) {
          ...
          return new Response(
            JSON.stringify({ error: "Missing goal data for creation" }),
            {
              status: 400,
              headers: { "Content-Type": "application/json", ...corsHeaders },
            },
      ...
      case "update": {
        if (!goalId || !goal) {
          ...
          return new Response(
            JSON.stringify({ error: "Missing goal ID or data for update" }),
            {
              status: 400,
              headers: { "Content-Type": "application/json", ...corsHeaders },
            },
      ...
      case "delete": {
        if (!goalId) {
          ...
          return new Response(
            JSON.stringify({ error: "Missing goal ID for deletion" }),
            {
              status: 400,
              headers: { "Content-Type": "application/json", ...corsHeaders },
            },
      ...
      case "fetch":
        if (goalId) {
          // Fetch single goal
          const { data: fetchedGoal, error: fetchSingleError } =
          ...
          const { data: fetchedGoals, error: fetchAllError } =
            await supabaseClient
              .from("goals")
              .select("*")
              .eq("user_id", user.id);
