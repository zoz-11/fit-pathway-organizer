...
const WorkoutReminderDataSchema = z.object({
  ...
  scheduledTime: z
    .string()
    .datetime("Scheduled time must be a valid datetime string"),
});
...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    if (!user) {
      await logAudit(supabaseClient, undefined, "Email Notification Failed", {
    ...
    const parsedBody = await NotificationEmailRequestSchema.parseAsync(
      await req.json(),
    );
    to = parsedBody.to;
    ...
    switch (type) {
      case "assignment":
        if (userRole !== "trainer") {
          return new Response(
            JSON.stringify({
              error: "Forbidden: Only trainers can send workout assignments",
            }),
            {
              status: 403,
              headers: { ...corsHeaders, "Content-Type": "application/json" },
      ...
      case "invitation":
        if (userRole !== "trainer") {
          return new Response(
            JSON.stringify({
              error: "Forbidden: Only trainers can send invitations",
            }),
            {
              status: 403,
              headers: { ...corsHeaders, "Content-Type": "application/json" },
      ...
      case "welcome":
        ...
        if (to !== userEmail) {
          return new Response(
            JSON.stringify({
              error: "Forbidden: Cannot send this notification to another user",
            }),
            {
              status: 403,
              headers: { ...corsHeaders, "Content-Type": "application/json" },
    ...
    switch (type) {
      case "workout_reminder":
        ...
        html = `
          ...
          <p>Hi there!</p>
          <p>Don't forget about your scheduled workout: <strong>${(data as WorkoutReminderData).workoutName}</strong></p>
          <p>Scheduled for: ${new Date((data as WorkoutReminderData).scheduledTime).toLocaleString()}</p>
        `;
        break;
      case "progress_update":
        ...
        html = `
          ...
          <p>Workouts completed this week: ${(data as ProgressUpdateData).workoutsCompleted}</p>
          <p>Total exercise time: ${(data as ProgressUpdateData).totalTime} minutes</p>
      ...
      case "welcome":
        ...
        html = `
          ...
          <p>Hi ${(data as WelcomeData).name}!</p>
          <p>We're excited to have you join our fitness community as a ${(data as WelcomeData).role}.</p>
      ...
      case "assignment":
        ...
        html = `
          ...
          <p><strong>${(data as AssignmentData).workoutName}</strong></p>
          <p>Description: ${(data as AssignmentData).description}</p>
      ...
      case "invitation":
        ...
        html = `
          ...
          <p>Hi there!</p>
          <p>You\'ve been invited by <strong>${(data as InvitationData).trainerName}</strong> to join FitPathway.</p>
    ...
    await logAudit(supabaseClient, user.id, "Email Notification Sent", {
      to: to,
      type: type,
