...
const Members = () => {
  ...
  if (isLoading) {
    return (
      <AppLayout>
        <PageLayout title={t('members.title')} description={t('members.description')}>
          ...
          <div className="flex gap-2">
            <Select value={statusFilter} onValueChange={setStatusFilter}>
              <SelectTrigger className="w-[180px]">
                <SelectValue placeholder={t('members.filter.status.placeholder')} />
              </SelectTrigger>
              <SelectContent>
                <SelectItem value="all">{t('members.filter.status.all')}</SelectItem>
                <SelectItem value="active">{t('members.filter.status.active')}</SelectItem>
                <SelectItem value="pending">{t('members.filter.status.pending')}</SelectItem>
              </SelectContent>
            </Select>
            <InviteAthleteDialog />
            <AssignWorkoutDialog />
              ...
              <CardHeader className="flex-row items-center gap-4">
                ...
                <DropdownMenu>
                  ...
                  <DropdownMenuContent align="end">
                    ...
                    <RemoveAthleteDialog athleteId={athlete.id} athleteName={athlete.full_name || t('members.athlete')} />
                  </DropdownMenuContent>
                </DropdownMenu>
              </CardHeader>
            </Card>
